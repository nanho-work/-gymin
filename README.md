...
현재 로그인, 로그아웃, 통계 일부, 이미지 업로드, 트레이너 프로필 등록/수정/내 프로필/상세 노출, 트레이너 구인글 지원/지원 현황, 사장님 지원자 목록/확인 신호는 FastAPI와 연결되어 있다. 구인글/센터/트레이너 목록 등 일부 화면은 아직 `web/src/data/mock/*.json` mock data를 사용한다.
