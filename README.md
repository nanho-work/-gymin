...
현재 로그인, 로그아웃, 홈 통계/최신 구인글, 이미지 업로드/삭제, 센터 등록/수정/상세, 구인글 목록, 트레이너 프로필 등록/수정/입력값 정규화/내 프로필/상세 노출, 트레이너 구인글 지원/지원 현황, 사업장 관리, 지원자 목록/확인 신호는 FastAPI와 연결되어 있다. 구인글 등록, 회원가입 목업 화면 등 일부 미완성 흐름은 아직 `MockField`를 사용한다.
