...
def create_job_endpoint(payload: JobPostCreate, db: Session = Depends(get_db)) -> JobPostRead:
    return create_job(db, payload)


@router.patch("/{job_id}/close", response_model=JobPostRead)
def close_job_endpoint(job_id: uuid.UUID, db: Session = Depends(get_db)) -> JobPostRead:
    job = get_job(db, job_id)
    if job is None:
        raise HTTPException(status_code=404, detail="구인글을 찾을 수 없습니다.")
